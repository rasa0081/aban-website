...
export default function ArticlePage() {
  ...
  useEffect(() => {
    ...
    const handleClick = (e) => {
      ...
      // Only handle links inside article content (identified by class)
      ...
      // 1. Anchor link — smooth scroll to the VISIBLE element
      // Mobile + desktop both render the same content, so there are two
      // elements with the same id. getElementById returns the first (mobile,
      // hidden at top:0). We find the visible one by checking getBoundingClientRect.
      if (href.startsWith('#')) {
      ...
      // 3. Relative internal link
      router.push(href);
    };

    // Use capture phase so we intercept before browser handles the link
  ...
  const fixContent = (html) => {
    if (!html) return html;
    // Remove target="_blank" and rel from anchor (#) links saved by old Tiptap
    let out = html.replace(/<a([^>]*)>/g, (match, attrs) => {
      const href = (attrs.match(/href="([^"]*)"/) || [])[1] || '';
      if (href.startsWith('#')) {
        // Strip target and rel for anchor links
        const cleaned = attrs
          .replace(/\s*target="[^"]*"/g, '')
          .replace(/\s*rel="[^"]*"/g, '');
        return `<a${cleaned}>`;
    ...
    // Force dir="rtl" on every block-level tag so mixed Persian/English/number
    // text doesn't get reordered by the browser's bidi algorithm (this fixes
    // both old articles that were saved with a stray dir="ltr"/"auto" and any
    // tag that never had a dir attribute at all).
    ...
    // Isolate bare Latin/number runs (e.g. "4p", "7p", "SMART") that sit
    // directly in RTL text with no wrapping tag of their own. Without this,
    // short mixed letter+digit tokens surrounded by Persian text can get
    // their characters/word-order scrambled by the browser's bidi algorithm.
    // We only touch text OUTSIDE existing tags AND outside HTML entities
    // (e.g. &nbsp;) — wrapping inside an entity would break it and make it
    // show up as literal visible text instead of being rendered.
    ...
    // Strip any inline font-family so all article text uses the site font (Veno),
    // which it inherits from the container. Pasted or imported content often
    // carries its own font-family inline, which would otherwise override the
    // site typography. Handles quoted ("Times New Roman") and unquoted values.
